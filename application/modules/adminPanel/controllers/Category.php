...
class Category extends Admin_Controller {
    ...
    public function add()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'GET') {
            ...
            $response = [
                ...
                if ($this->input->post('age')['min'] >= $this->input->post('age')['max'])
                    die(json_encode([ 'message' => "Min Age must less than Max Age.", 'status' => false ]));
                
            	$post = [
	                    'cat_name'      => $this->input->post('cat_name'),
	                    'min_age'       => $this->input->post('age')['min'],
	                    'max_age'       => $this->input->post('age')['max'],
                        'time_duration' => $this->input->post('time'),
    ...
    public function update($id)
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'GET') {
            ...
            $data['url'] = $this->redirect;
            $data['id'] = $id;
            $data['data'] = $this->main->get($this->table, 'cat_name, min_age, max_age, time_duration, price', ['id' => d_id($id)]);
